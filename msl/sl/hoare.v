...
Definition world_age : ageable world :=
  prod_ageable nat (locals * mem) nat_ageable.
Existing Instance world_age.

Definition world_asa : ASA world :=
 asa_prod nat (locals * mem) (sa_equiv nat) nat_ageable (sa_prod locals_sa mem_sa) nat_ASA.
...
Definition func_pre (fd:fun_decl) (fs:fun_spec) (a:fs_A fs) : pred world :=
  Ex_ vs:list val, fs_pre fs a vs * func_start_locals fd vs.
...
Definition satisfies_fun_spec (pu:program_unit) (i:ident) (fs:fun_spec) : pred nat :=
  Ex_ fd:fun_decl, !!(pu i = Some fd) &&
    All_ a:(fs_A fs), |>hoare_ pu (func_post fs a) (func_pre fd fs a) (fnd_cmd fd) FF.
...
Definition satisfies_spec (pu:program_unit) (ps:prog_spec) : pred nat :=
    All_ i:ident, All_ fs:fun_spec, !!(ps i = Some fs) --> satisfies_fun_spec pu i fs.
...
Proof.
  ...
  rewrite and_comm.
  rewrite TT_and.
  rewrite and_comm in H.
  rewrite TT_and in H.
  auto.
  rewrite and_comm in H.
  rewrite FF_and in H.
  rewrite FF_con in H. elim H.
  simpl list_eval in *.
  destruct vals.
  rewrite and_comm in H.
  rewrite FF_and in H.
...
(* The make_locals function actually satisfies teh func_start_locals predicate. *)
...
Proof.
  ...
  simpl snd. split.
  simpl fst.
  apply LocalsMap.build_map_join.
  simpl. auto.
  ...
  simpl. unfold LocalsMapInput.A_dec_eq.
  destruct a. simpl.
  ...
  destruct a; simpl.
  unfold LocalsMapInput.A_dec_eq.
...
(* mapsto implies the ability to read from memory *)
...
Proof.
  ...
  rewrite <- H8 in H0.
  discriminate.
  unfold MemMap.map_val. unfold lookup_fpm.
  rewrite <- H10. auto.
  unfold MemMap.map_val in H0.
  unfold lookup_fpm in H0.
  destruct w1 as [? [? ?]].
  ...
  unfold MemMap.map_val. unfold lookup_fpm.
  rewrite  <- H9.
  destruct z. destruct x. rewrite <- H0.
  destruct y. simpl in H10.
  inv H10.
  inv H3. congruence.
Qed.

(* mapsto with the full share implies that
...
Proof.
  ...
  spec H4. split.
  intros e rho' m' w ? ?.
  rewrite ex_con in H9.
  destruct H9 as [v ?].
  rewrite and_comm in H9.
  apply eval_up' in H9.
  destruct H9.
...
(* existentials in the precondition are equivalant to
   universals bound outside the hoare tuple.
 *)
Lemma hoare_ex : forall ps R A (P:A->pred world) c Q,
  hoare_ ps R (existential P) c Q =
  (All_ x:A, hoare_ ps R (P x) c Q).
Proof.
...
Proof.
  intros. rewrite <- impl_and_adjoint.
  intros x H.
  destruct H.
  intros k F x' H1 H2 rho m x'' H3 H4.
...
Proof.
  intros. do 10 intro.
  destruct H1.
  repeat rewrite con_assoc in H3.
  ...
  repeat intro.
  eapply H1; eauto.
  rewrite ex_con in H8.
  rewrite ex_con.
  destruct H8 as [v ?]. exists v.
  rewrite and_comm in H8.
  rewrite <- con_assoc in H8.
  revert H8.
  apply split_con.
  repeat intro.
  apply eval_up' in H8.
...
Lemma hoare_call : forall ps R i f es fs Q n,
  hoare_ ps R
     ...
     (Ex_ vals:list val, list_eval es vals &&
       Ex_ a:fs_A fs,
         fs_pre fs a vals *
         defined i *
         All_ v:val,
...
Proof.
  ...
  assert ((fs_post fs a v -* Q)%pred (n'0,(rho'',wQm))).
    ...
    destruct H1.
    destruct H1; auto.
    destruct H0. destruct H0; simpl in *; congruence.
    ...
    clear.
    change (hasval i0 v * (All_ v0:val, fs_post fs a v0 * hasval i0 v0 -* Q) |-- fs_post fs a v -* Q).
    rewrite <- wand_con_adjoint.
    rewrite con_comm.
    rewrite <- con_assoc.
    apply derives_cut
      with (fs_post fs a v * hasval i0 v *
        (fs_post fs a v * hasval i0 v -* Q)).
    apply split_con. hnf; auto.
    do 2 intro.
    spec H v. auto.
    rewrite con_comm.
    rewrite wand_con_adjoint. hnf; auto.

  rewrite and_comm in H2. apply eval_up' in H2.
  destruct H2.
  unfold func_post in H2.
  rewrite con_comm in H2.
  rewrite <- con_assoc in H2.
  ...
  repeat intro. rewrite FF_con in H3. elim H3.
...
Lemma hoare_if : forall ps R e P c1 c2 Q n,
  hoare_ ps R (Ex_ v:nat, eval e (int_val v) && !!(v <> 0) && P) c1 Q n ->
  hoare_ ps R (eval e (int_val 0) && P) c2 Q n ->

  hoare_ ps R
      (Ex_ v:nat, eval e (int_val v) && P)
      (cif e c1 c2)
      Q n.
Proof.
  intros.
  do 10 intro.
  rewrite ex_con in H4.
  destruct H4 as [v ?].
  destruct H4 as [w1 [w2 [? [? ?]]]].
  destruct H5.
...
Lemma hoare_assign : forall ps R i e Q n,
   hoare_ ps R
     (Ex_ v:val, eval e v && (defined i * (hasval i v -* Q)))
     (assign i e)
     Q n.
Proof.
  intros. apply hoare_straight. intros.
  rewrite ex_con in H0.
  destruct H0 as [v ?].
  rewrite and_comm in H0.
  apply eval_up' in H0.
  destruct H0.
  rewrite con_assoc in H0.
...
Lemma hoare_store : forall ps R e1 e2 Q n,
  hoare_ ps R
     (Ex_ a:addr, Ex_ v:val,
       eval e1 (ptr_val a) &&
       eval e2 v &&
       ( (Ex_ vold:val, mapsto a Share.top vold) *
...
Lemma hoare_load : forall ps R i e Q n,
  hoare_ ps R
     (Ex_ a:addr, Ex_ sh:Share.t, Ex_ v:val,
                  eval e (ptr_val a) &&
...
Proof.
  ...
  apply eval_up' in H.
  destruct H.
  rewrite (con_comm _ (defined i)) in H.
  repeat rewrite con_assoc in H.
  ...
  econstructor; eauto.
  hnf in H0. eauto.
  rewrite con_comm in H1.
  repeat rewrite con_assoc in H1.
  eapply mapsto_val; eauto.
  repeat rewrite <- con_assoc in H1.
  generalize H1.
  apply split_con.
  rewrite (con_comm (hasval i v) _).
  rewrite con_comm.
  rewrite wand_con_adjoint. hnf; auto.
  hnf; auto.
Qed.
...
Proof.
  ...
  rewrite nec_nat. omega.
  destruct H4. destruct H4. destruct w2; simpl in *; congruence.
