...
Definition can_weaken_env_fresh f := forall s1 s2 h1 h2 R x u,
  ~ Fmap.indom s1 x ->
  ~ Fmap.indom s2 x ->
  satisfies s1 s2 h1 h2 R f ->
  satisfies (Fmap.update s1 x u) (Fmap.update s2 x u) h1 h2 R f.

Definition can_weaken_env_except x0 f := forall s1 s2 h1 h2 R x u,
  x <> x0 ->
  satisfies s1 s2 h1 h2 R f ->
  satisfies (Fmap.update s1 x u) (Fmap.update s2 x u) h1 h2 R f.
...
Proof.
  ...
  inverts H1 as H1.
  unfold Fmap.update in H0.
  lets: Fmap.disjoint_single_of_not_indom u0 H0.
  apply s_defun.
  { apply* not_indom_update.
    rewrite Fmap.disjoint_union_eq_r in H2. destr H2.
    apply disjoint_single_neq in H3.
    congruence. }
  { unfold Fmap.update.
    fmap_eq. }
Qed. *)
...
Inductive bigstep : penv -> heap -> expr -> penv -> heap -> eresult -> Prop :=
  ...
  | eval_padd : forall p1 h v1 v2 i1 i2 e1 e2,
    vint i1 = v1 ->
    vint i2 = v2 ->
  ...
  | eval_plet_sh : forall x e1 e2 h1 h2 p1 p2 k eb x ek y x1,
...
Example e3_shift_k_k : forall k x1 x, exists Re,
  bigstep empty_penv empty_heap
    (preset (plet x1
      (pshift k (pval (vfun x (papp (pvar k) (pvar x)))))
      (padd 1 (pvar x1))))
    empty_penv empty_heap Re.
Proof.
  intros. eexists.
