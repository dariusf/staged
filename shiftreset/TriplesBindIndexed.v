...
Inductive spec_assert_valid_under penv (env:senv) : int -> expr -> flow -> Prop :=
  | sav_base: forall e f,
    ...
    spec_assert_valid_under penv env 0 e f

| sav_shift: forall n e f, n > 0 ->
    (forall h1 h2 v, not (bigstep penv h1 e h2 (enorm v))) ->
    (forall h1 h2 (eb:var->expr) (ek:val->expr),
        bigstep penv h1 e h2 (eshft eb ek) -> (* get the n from here? *)
        exists (fb:var->flow) (fk:val->flow),
          satisfies env env h1 h2 (shft fb fk) f /\
            (forall x, spec_assert_valid_under penv env (n-1) (eb x) (fb x)) /\
            (forall v, spec_assert_valid_under penv env (n-1) (ek v) (fk v))) ->
    spec_assert_valid_under penv env n e f.
...
Definition spec_assert_valid n e f : Prop :=
  forall penv env,
    spec_assert_valid_under penv env n e f.


Lemma pval_sound: forall v,
  spec_assert_valid 0 (pval v) (ens (fun r => \[r = v])).
Proof.
...
Lemma pshift_sound: forall eb fb,
  (forall n k, spec_assert_valid n (eb k) (fb k)) ->
  spec_assert_valid 1 (pshift eb) (sh fb).
Proof.
  unfold spec_assert_valid. intros * Heb **.
  applys sav_shift. math. { introv H. false_invert H. } intros.
  inverts H.
  exs.
  splits.
  { applys s_sh. }
...
Lemma papp_unk_sound: forall penv (env:senv) (f:var) v (ef:val->expr) (f1:val->flow) n,
  Fmap.read penv f = ef ->
  Fmap.read env f = f1 ->
  spec_assert_valid_under penv env n (ef v) (f1 v) ->
  spec_assert_valid_under penv env n (papp f v) (unk f v).
Proof.
...
Lemma papp_sound: forall x e (v:val) f n,
  spec_assert_valid n (subst x v e) f ->
  spec_assert_valid n (papp (vfun x e) v) f.
Proof.
...
Lemma plet_sound: forall n n1 x e1 e2 f1 (f2:val->flow),
  spec_assert_valid n1 e1 f1 ->
  n >= n1 ->
  (forall v, spec_assert_valid (n-n1) (subst x v e2) (f2 v)) ->
  spec_assert_valid n (plet x e1 e2) (bind f1 f2).
Proof.
  intros n.
  induction_wf IH: (downto 0) n.
  unfold spec_assert_valid in *.
  introv He1 Hwf He2. intros. specializes He1 penv0 env.
  inverts He1 as.
  {
    (* e1 has no shift *)
    introv Hne1 Hb1.
  ...
  }
  {
    ...
    { intros.
      ...
      } *)
      {
        ...
        (* assert (n > 0) as ?. admit. *)
        unfold downto. math.
        specializes IH (n1-1) x (ek0 v) e2 (fk v).
        specializes IH f2.
        applys IH.
        admit.
        math.
        intros.
        specializes He2 v0 penv1 env0.
        admit.
