...
(* Definition ContT (r:Type) (m:Type->Type) (a:Type) := (a -> m r) -> m r.

Definition contt_ret (r:Type) (m:Type->Type) (a:Type) (x:a): ContT r m a :=
  fun k => k x.

Definition contt_bind (r:Type) (m:Type->Type) (a b:Type)
  (m1:ContT r m a) (f:a -> ContT r m b): ContT r m b :=
...
Definition ContP (r a : Type) := (a -> PP r) -> PP r.

Definition contp_ret (r a:Type) (x:a): ContP r a :=
  fun k => k x.

Definition contp_bind (r a b:Type) (m:ContP r a) (f:a -> ContP r b): ContP r b :=
  fun c => m (fun a => f a c).

Definition contp_eval (a:Type) (k : ContP a a) : PP a := k (fun x => pp_ret x).

Definition contp_shift (a r:Type) (f:(a -> PP r) -> PP r): ContP r a := f.

Definition contp_reset (a:Type) (f: ContP a a): PP a := contp_eval f.


Definition entails (r a:Type) (s1 s2:ContP a a): Prop :=
  refines (contp_eval s1) (contp_eval s2).


Notation "x <- c1 ;; c2" := (contp_bind c1 (fun x => c2))
                             (right associativity, at level 84, c1 at next level).

Notation "c1 ;; c2" := (contp_bind c1 (fun _ => c2)) (at level 100, right associativity).


(* Definition ensure (r a:Type) (q:a -> Prop) : ContP r a :=

  fun c => (pre_post True (fun v => q v))
  . *)
...
(* Definition ensure_ (r:Type) (q:Prop) : ContP r unit :=
  cont_ret (pre_post True (fun _ => q)).

Definition require (r:Type) (p:Prop) : ContP r unit :=
