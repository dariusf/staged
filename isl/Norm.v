...
From ISL Require Import Basics Satisfies Propriety.
...
Lemma norm_reassoc : forall H f1 f2,
  (* shift_free f1 -> *)
  entailed (req H (f1;; f2)) (req H f1;; f2).
Proof.
  unfold entailed.
  intros.
  inverts H0.
...
Proof.
  unfold entailed. apply satisfies_ens_ens_void_combine.
Qed.

Lemma norm_ens_ens_void_combine : forall H1 H2,
  entailed (ens_ H1;; ens_ H2) (ens_ (H1 \* H2)).
Proof.
  unfold entailed; apply satisfies_ens_ens_void_split.
Qed.

Lemma norm_bind_assoc_conv: forall f fk fk1,
  entailed (bind f (fun r => bind (fk r) fk1))
    (bind (bind f fk) fk1).
Proof.
  unfold entailed. intros * H.
  inverts H.
  { inverts H7.
    applys s_bind H6.
    applys* s_bind H9. }
...
Lemma norm_req_sep_combine : forall H1 H2 f,
  entailed (req H1 (req H2 f)) (req (H1 \* H2) f).
Proof.
  unfold entailed.
  intros.
...
Lemma norm_req_sep_split : forall H1 H2 f,
  entailed (req (H1 \* H2) f) (req H1 (req H2 f)).
Proof.
  unfold entailed.
  intros.
...
Lemma norm_rs_req : forall H f,
  entailed (rs (req H f)) (req H (rs f)).
Proof.
  unfold entailed. intros.
  apply s_req. intros.
  inverts H0 as H0.
...
Lemma norm_rs_seq_ens : forall Q f,
  entailed (rs (ens Q;; f)) (ens Q;; (rs f)).
Proof.
  unfold entailed. intros.
  apply red_skip.
  apply sf_ens.
...
Lemma norm_rs_seq_ens_void : forall H f,
  entailed (rs (ens_ H;; f)) (ens_ H;; (rs f)).
Proof.
  unfold entailed. intros.
  apply red_skip.
  apply sf_ens.
...
Lemma entails_ens_seq : forall H1 H2 f1 f2,
  H1 ==> H2 ->
  entailed f1 f2 ->
  entailed (ens_ H1;; f1) (ens_ H2;; f2).
Proof.
  unfold entailed.
  intros.
...
Lemma norm_seq_all_r: forall (A:Type) (f:A->flow) f1,
  shift_free f1 ->
  entailed (f1;; (∀ x:A, f x)) (∀ x:A, f1;; f x).
Proof.
  unfold entailed. introv Hsf H0.
...
Lemma norm_rs_ex : forall A ctx,
  entailed (rs (∃ (x:A), ctx x)) (∃ (x:A), rs (ctx x)).
Proof.
  unfold entailed. intros.
  inverts H as H.
  {
...
Lemma norm_req_all : forall H (A:Type) (fctx:A->flow),
  entailed (req H (∀ b, fctx b)) (∀ b, req H (fctx b)).
Proof.
  unfold entailed. intros.
  applys s_fall. intros.
  applys s_req. intros.
...
Lemma norm_seq_ex_l : forall f1 (A:Type) (fctx:A->flow),
  entailed ((∃ b, fctx b);; f1) (∃ b, fctx b;; f1).
Proof.
  unfold entailed. intros.
  inverts H.
  { inverts H7. destr H5.
...
Lemma norm_bind_seq : forall f fk v,
  ...
  det f ->
  flow_res f v ->
  entailed (bind f fk) (f;; fk v).
Proof.
  unfold flow_res, entailed. intros * Hsf Hd Hfr * H.
...
Lemma norm_bind_seq1 : forall f fk v,
  ...
  entailed (bind f fk) (f;; fk v).
Proof.
  unfold entailed. intros * Hsf Hfr * H.
  inverts H. 2: { false Hsf H6. }
  specializes Hfr H7.
  subst.
  applys* s_seq H7.
...
Lemma norm_bind_seq_assoc : forall fk f1 f2,
  shift_free f1 ->
  entailed (bind (f1;; f2) fk) (f1;; bind f2 fk).
Proof.
  unfold entailed. intros * Hsf1 * H.
  inverts H.
  { inverts H7.
    applys s_seq H6.
    applys* s_bind. }
...
Lemma norm_bind_val : forall fk v,
  entailed (bind (ens (fun r => \[r = v])) fk) (fk v).
Proof.
  unfold entailed. intros * H.
...
Lemma norm_bind_disj_val : forall fk v1 v2,
  entailed (bind (ens (fun r => \[r = v1 \/ r = v2])) fk)
    (disj (fk v1) (fk v2)).
Proof.
  unfold entailed. intros * H.
  inverts H. 2: { false sf_ens H6. }
...
(* Lemma norm_bind_pure : forall fk (P:val->Prop) Q,
  entailed (bind (ens (fun r => \[P r])) (fun v => ens (Q)))
    (ens (fun r => Q r * \[P r])).
Proof.
  unfold entailed. intros * H.
  inverts H. 2: { false sf_ens H6. }
...
Lemma norm_bind_ens_void : forall fk H,
  entailed (bind (ens_ H) fk) (seq (ens_ H) (fk vunit)).
Proof.
  unfold entailed. intros * H.
  inverts H.
  { pose proof H8.
...
Lemma norm_bind_req : forall H f fk,
  entailed (bind (req H f) fk) (req H (bind f fk)).
Proof.
  unfold entailed. intros * H.
  applys s_req. intros.
  inverts H.
...
Lemma norm_bind_disj: forall f1 f2 fk,
  entailed (bind (disj f1 f2) fk) (disj (bind f1 fk) (bind f2 fk)).
Proof.
  unfold entailed. intros.
  inverts H.
  { inverts H7.
...
Lemma norm_rs_disj: forall f1 f2,
  entailed (rs (disj f1 f2)) (disj (rs f1) (rs f2)).
Proof.
...
Lemma norm_seq_disj_r : forall f1 f2 f3,
  shift_free f1 ->
  entailed (f1;; disj f2 f3)
    (disj (f1;; f2) (f1;; f3)).
Proof.
  unfold entailed. introv Hsf H.
  inverts H. 2: { false Hsf H6. }
  inverts H8.
...
Lemma norm_seq_ens_ens_pure : forall H P,
  entailed (ens_ H;; ens_ \[P]) (ens_ \[P];; ens_ H).
Proof.
  unfold entailed. intros.
  inverts H0. 2: no_shift.
  inverts H8.
...
Lemma norm_bind_all_l : forall (A:Type) (fk1:A->flow) fk,
  entailed (bind (∀ (b:A), fk1 b) fk) (∀ (b:A), bind (fk1 b) fk).
Proof.
  unfold entailed. introv H.
  inverts H.
...
Lemma norm_bind_all_r1 : forall (A:Type) f (fk:A->val->flow),
  shift_free f ->
  entailed (bind f (fun r => ∀ (x:A), fk x r)) (∀ (x:A), bind f (fk x)).
Proof.
  unfold entailed. introv Hsf H.
  inverts H. 2: { false Hsf H6. }
  inverts H8.
...
Lemma norm_bind_seq_def: forall f1 f2,
  entailed (bind f1 (fun _ => f2)) (f1;; f2).
Proof.
  intros.
  fold (f1;; f2).
  reflexivity.
...
Lemma norm_defun_discard_id1: forall (f:var) u,
  entailed (defun f u;; discard f) empty.
Proof.
  unfold entailed. intros.
  inverts* H.
  inverts H7.
...
Lemma norm_seq_defun_discard1: forall (f:var) u f1,
  entailed (defun f u;; discard f;; f1) f1.
Proof.
  intros.
  rewrite* norm_seq_assoc_sf.
...
Lemma norm_seq_defun_ens_void: forall (f:var) u H,
  entailed (defun f u;; ens_ H) (ens_ H;; defun f u).
Proof.
  unfold entailed. intros.
  inverts* H0.
  inverts H8.
...
Lemma norm_seq_defun_rs: forall (f:var) f1 u,
  entailed (defun f u;; rs f1) (rs (defun f u;; f1)).
Proof.
  unfold entailed. intros.
  inverts* H.
  inverts H7.
...
Lemma norm_seq_defun_bind_l: forall (f:var) f1 fk u,
  entailed (defun f u;; bind f1 fk) (bind (defun f u;; f1) fk).
Proof.
  unfold entailed. intros.
  inverts* H.
  inverts H7.
...
Lemma norm_bind_seq_defun_ens: forall (f:var) Q fk u,
  entailed (bind (defun f u;; ens Q) fk)
...
Lemma norm_bind_rs_seq_defun_ens: forall (f:var) Q fk u,
  entailed (bind (rs (defun f u;; ens Q)) fk)
...
Lemma norm_seq_defun_req: forall (f:var) u f1 H,
  entailed (defun f u;; req H f1) (req H (defun f u;; f1)).
Proof.
  unfold entailed. intros.
  inverts H0. 2: no_shift.
  inverts H8.
...
Lemma norm_seq_defun_unk: forall (f:var) u v,
  entailed (defun f u;; unk f v) (defun f u;; u v).
Proof.
  unfold entailed. intros.
  inverts H. 2: no_shift.
  inverts H7.
...
Lemma norm_bind_seq_past_pure_sf: forall f1 fk P,
  shift_free f1 ->
  entailed (bind (ens (fun r => \[P r])) (fun r => f1;; fk r))
...
Lemma norm_bind_ens_req: forall P fk H,
  entailed (bind (ens (fun r => \[P r])) (fun r => req H (fk r)))
...
Lemma norm_ens_hstar_pure_r: forall H (P:val->Prop),
  entailed (ens (fun r => H \* \[P r])) (ens_ H;; ens (fun r => \[P r])).
Proof.
  unfold entailed. intros.
  inverts H0.
  heaps.
...
Lemma norm_bind_trivial_sf: forall f1,
  shift_free f1 ->
  entailed (bind f1 (fun r2 => ens (fun r1 => \[r1 = r2]))) f1.
Proof.
  unfold entailed. introv Hsf H.
  inverts* H.
  inverts H8. heaps.
...
Lemma norm_seq_defun_skip_ens_void: forall (f:var) u H f1,
  entailed (defun f u;; ens_ H;; f1) (ens_ H;; defun f u;; f1).
Proof.
  unfold entailed. intros.
  inverts* H0.
  inverts H8.
...
Lemma norm_seq_defun_all: forall (f:var) u (A:Type) (P:A->flow),
  entailed (defun f u;; ∀ x, P x) (∀ x, defun f u;; P x).
Proof.
  unfold entailed. intros.
  inverts* H.
  inverts H7.
...
Lemma norm_rs_seq_defun_out: forall (f:var) f1 u,
  entailed (rs (defun f u;; f1)) (defun f u;; rs f1).
Proof.
  unfold entailed. intros.
  inverts* H.
  { inverts* H1.
