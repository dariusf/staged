...
Section Properties.
  ...
  Context {ArrEq : Equiv2 Arr} {AA : Arrow Arr}.
  Context {Sub : Obj → Obj → Type} {EqSub : EqCore2 Sub} {SC : ArrowCore Sub}.
  Context {ASC : SubstCore Arr Sub} {SubEq : Equiv2 Sub} {AS : Arrow Sub} {SS : Subst Arr Sub}.
  ...
  Global Instance fmap_proper A B : Proper (equal ==> eq ==> eq) (fmap (F := G) (A := A) (B := B)).
  Proof.
  ...
  Global Instance bind_proper A B : Proper (equal ==> eq ==> eq) (bind (A := A) (B := B)).
  Proof.
    intros f g EQ t t' EQ'; subst t'.
