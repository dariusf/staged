Require Import Utf8.
Require Import Binding.Equality.
...
Ltac auto_map_id :=
  ...
  | |- ?f ≡ ı → fmap ?f ?t = ?t =>
      let EQ := fresh "EQ"
      in intros EQ; simpl in EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try apply EQ;
              now eauto using @map_id, @lift_id with typeclass_instances)
  end.
...
Ltac auto_map_comp :=
  ...
  | |- ?f ∘ ?g ≡ ?h → fmap ?f (fmap ?g ?t) = fmap ?h ?t =>
      ...
      in intros EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try apply EQ;
              now eauto using @map_map_comp, @lift_comp with typeclass_instances)
  end.
...
Ltac auto_map_bind_pure :=
  ...
  | |- ?f ̂ ≡ ?g → fmap ?f ?t = bind ?g ?t =>
      ...
      in intros EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try apply EQ;
              try match goal with
              | [ H: ∀ A B f g e, (f ̂)%bind ≡ g → fmap f e = bind g e |- _ ] =>
                apply H
              end;
              now eauto using @bind_map, @lift_of, lift_of
...
Ltac auto_map_bind_comm :=
  ...
  | |- ?g₂ ∘ ?f₂ ̂ ≡ ?f₁ ̂ ∘ ?g₁ → bind ?g₂ (fmap ?f₂ ?t) = fmap ?f₁ (bind ?g₁ ?t) =>
      let EQ := fresh "EQ"
      in intros EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try (rewrite map_to_bind; apply EQ);
              try match goal with
              | [ H: ∀ A B₁ B₂ C f₁ f₂ g₁ g₂ e,
                      (g₂ ∘ f₂ ̂)%bind ≡ (f₁ ̂ ∘ g₁)%bind
                      → bind g₂ (fmap f₂ e) = fmap f₁ (bind g₁ e) |- _ ] =>
                apply H
              end;
              now eauto using @bind_map_comm, @lift_comm, lift_comm
                with typeclass_instances)
  end.
...
Ltac auto_bind_id :=
  ...
  | |- ?f ≡ ı → bind ?f ?t = ?t =>
      ...
      in intros EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try apply EQ;
              try match goal with
              | [ H: ∀ A f e, f ≡ ı%bind → bind f e = e |- _ ] => apply H
              end;
              now eauto using @bind_pure, @lift_id, lift_id
                with typeclass_instances)
  end.
  ...
  | |- ?f ∘ ?g ≡ ?h → bind ?f (bind ?g ?t) = bind ?h ?t =>
      ...
      in intros EQ; destruct t; term_simpl;
         try (repeat (progress f_equal); try apply EQ;
              try match goal with
              | [ H: ∀ A B C f g h e,
                  (f ∘ g)%bind ≡ h → bind f (bind g e) = bind h e |- _ ] =>
                apply H
              end;
              now eauto using @bind_bind_comp, @lift_comp,
                lift_comp with typeclass_instances)
  end.
