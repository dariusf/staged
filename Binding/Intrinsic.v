Require Import Utf8.
Require Import Binding.Equality Binding.Core.
Require Import Binding.Inc Binding.Set.
...
Instance ArrEqC {V} : EqCore2 (Arr (V := V)) :=
  λ A B f g, ∀ x, f x = g x.

Instance ArrEq {V} : Equiv2 (Arr (V := V)).
Proof.
  split; intros; congruence.
Qed.

Instance SubEqC {V F} {FP : IntPureCore (V := V) F} : EqCore2 Sub :=
  λ A B f g, ∀ v x EQ, f v x EQ = g v x EQ.

Instance SubEq {V F} {FP : IntPureCore (V := V) F} : Equiv2 Sub.
Proof.
  split; intros; congruence.
Qed.
...
Section Reindexing.
  ...
  Definition comp (f : U → V) (Δ : Ctx U) : Ctx V :=
    {| dom := dom Δ;
      arr x := f (arr Δ x) |}.

  Definition arrComp {X Y} (f : U → V) (δ : X [→] Y) : comp f X [→] comp f Y :=
    {| apply_arr := δ : dom (comp f X) → dom (comp f Y);
       arr_hom x := f_equal f (arr_hom δ x) |}.

End Reindexing.

Notation "f • Γ" := (comp f Γ) (at level 45, right associativity) : bind_scope.
Notation "f •ᵃ δ" := (arrComp f δ) (at level 45, right associativity) : bind_scope.

Module Forget.
  Local Open Scope bind_scope.

  Notation "X [→]ˢ Y" := (Binding.Set.Arr X Y) (at level 45, right associativity) : type_scope.

  Definition forget {V} {X Y : Ctx V} (δ : X [→] Y) : dom X [→]ˢ dom Y :=
    {| Binding.Set.apply_arr := δ |}.
  Notation "⌊ δ ⌋" := (forget δ) : bind_scope.

  Lemma lift_drop {V} {X Y : Ctx V} v (δ : X [→] Y) :
    ⌊ δ ⌋ ↑ ≡ (⌊ δ ↑ ⌋ : dom (X ▹ v) [→]ˢ dom (Y ▹ v)).
  Proof.
    now intros [| x].
  Qed.

  Lemma mcomp_drop {U V} (f : U → V) {X Y : Ctx U} (δ : X [→] Y) : ⌊f •ᵃ δ⌋ ≡ ⌊δ⌋.
  Proof.
    reflexivity.
  Qed.
