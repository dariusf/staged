...
Require Import msl_lambda.lam_ref_type_prelim.
...
Lemma expr_type_val' : forall tau,
  TT |-- All_ v:value, All_ e:expr, %(expr_type e tau --> with_val v (expr_type e tau)).
Proof.
...
Proof.
  ...
  set (f' := fun a => if beq_nat a l then Some (app_pred tau) else f a).
  ...
  split; auto.
  change (level x' < n).
  apply le_lt_trans with (level a'3).
  ...
  simpl in H11.
  apply extends_fashionable in H11.
  auto.
