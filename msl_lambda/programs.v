...
Require Import msl_lambda.lam_ref_eval.
...
Require Import msl_lambda.lam_ref_type_rules.
...
Definition ty_bool := All_ a:pred world, ty_lam a (ty_lam a a).
...
Definition option (a:pred world) :=
  All_ b:pred world, ty_lam b (ty_lam (ty_lam a b) b).
...
Lemma out_typ :
  Typ nil out (All_ tau:pred world, ty_lam tau (ty_lam (option tau) tau)).
Proof.
  intros.
  unfold out.
  apply T_UnivI; simpl; intros; auto.
...
Lemma none_typ :
  Typ nil none (All_ tau:pred world, option tau).
Proof.
  intros.
  unfold none.
  unfold option.
...
Lemma some_typ :
  Typ nil some (All_ tau:pred world, ty_lam tau (option tau)).
Proof.
  unfold some.
  apply T_UnivI; simpl; intros; auto.
  apply T_Abs.
...
Lemma W_typ : forall Z, Typ nil W (Rec (W_ty_cont Z)).
Proof.
  intros.
  ...
  unfold W_ty.
  apply T_Abs.
  apply T_App with (Rec (W_ty_cont Z)).
  rewrite Rec_fold_unfold.
  apply T_Var; auto.
  apply T_Var; auto.
Qed.
...
Proof.
  intros.
  unfold diverge.
  apply T_App with (Rec (W_ty_cont t)).
  generalize (W_typ t).
...
Lemma Wf_typ : forall A B,
  Typ (ty_lam (ty_lam A B) (ty_lam A B) :: nil) Wf (Rec (Wf_ty_cont A B)).
Proof.
  intros.
  unfold Wf.
  rewrite Rec_fold_unfold.
  ...
  apply T_Abs.
  apply T_App with A.
  apply T_App with (Rec (Wf_ty_cont A B)).
...
Definition U := Rec U_contractive.
