...
(** Here we prove that the "obvious" typing rule
    for universal introduction is incorrect.
...
Proof.
  ...
  unfold crash.
  unfold e_let at 1.
  apply T_App with (All_ tau:pred world, ty_lam tau (ty_ref (option tau))).
  apply T_Abs.
  unfold e_let at 1.
...
(* In contrast, the following slight modification is
   ...
   the universal introduction rule with the
   value restriction.
 *)
...
Proof.
  ...
  unfold ok_program.
  unfold e_let at 1.
  apply T_App with (All_ tau:pred world, ty_lam ty_nat (ty_lam tau (ty_ref (option tau)))).
  apply T_Abs.
  unfold e_let at 1.
  apply T_App with (ty_ref (option (ty_lam ty_nat ty_nat))).
  apply T_Abs.
