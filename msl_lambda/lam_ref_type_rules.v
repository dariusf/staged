...
Require Import msl_lambda.lam_ref_type_defs.
Require Import msl_lambda.lam_ref_type_safety.
Require Import msl_lambda.lam_ref_type_lemmas.
...
Proof.
  ...
  apply positive_boxy.
  hnf; rewrite box_and.
  rewrite later_commute; rewrite box_refl_trans; auto.
  rewrite fashionable_extends; auto.
  ...
  auto.
  apply H2.
  instantiate (1:= (tau >=> tau')); auto.
  apply H1.
  hnf; rewrite fashionable_extends; auto.
  ...
  spec H4.
  split; auto.
  spec H2 (m0,exp_to_val e H).
  apply H2; auto.
  simpl.
  rewrite <- fashionable_same_level.
  hnf in H5.
  repeat rewrite knot_level.
  destruct (unsquash k); destruct (unsquash  m0); intuition.
...
Proof.
  ...
  cut (TT |-- %(All_ e:expr, !!(closed e) && expr_type e tau --> expr_type (New e) (ty_ref tau))).
  intros.
  eapply H3; eauto.
  2: apply R_extends_refl.
  simpl; auto.
...
Proof.
  ...
  spec H1 env a H3.
  spec H2 env a H3.
  cut (TT |-- %(All_ e1:expr, All_ e2:expr, (!!(closed e1 /\ closed e2)) && expr_type e1 (ty_lam sigma tau) && expr_type e2 sigma --> expr_type (App e1 e2) tau)).
...
Proof.
  ...
  cut ( TT |-- %(All_ e:expr, !!(closed e) && expr_type e (ty_ref tau) --> expr_type (Deref e) tau)).
  intros.
  eapply H2.
  2: apply R_extends_refl.
  2: apply rt_refl.
...
Proof.
  ...
  cut ( TT |-- %(All_ e1:expr, All_ e2:expr, All_ e3:expr,
...
Definition AllBnd (T:pred world) (X:pred world -> pred world)
  := All_ tau:pred world, !!(TT |-- tau >=> T) --> X tau.
...
Proof.
  ...
  simpl in H2.
  eapply H0; eauto.
  2: simpl;  apply fashionR_refl.
  simpl; auto.
  apply H1; auto.
  simpl; auto.
  apply sub_trans with T2; auto.
Qed.

Lemma AllBnd_All : forall (X:pred world -> pred world),
  universal X = AllBnd TT X.
Proof.
  intros.
...
Lemma T_UnivE : forall G e (X:pred world -> pred world) tau,
  ...
  Typ G e (X tau).
Proof.
  do 4 intro; apply T_sub; apply sub_forall_spec.
Qed.


Definition ty_ex (F:pred world -> pred world) : pred world :=
  Ex_ tau:pred world, !!(boxy extendM tau) && F tau.
