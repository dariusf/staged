...
Definition new (m : mem) (v : value) : (mem * addr) :=
  match m with (n, m') =>
    ((S n, fun a => if Nat.eqb a n then v else m' a), n)
  end.
...
Definition update (m : mem) (a : addr) (v : value) : mem :=
  match m with (n, m') =>
    (n, fun a' => if Nat.eqb a a' then v else m' a')
  end.
...
Fixpoint subst (var : var_t) (v : value) (e : expr) : expr :=
  match e with
   ...
   | Loc l => Loc l
   | Var var' => if (Nat.eqb var var') then val_to_exp v else Var var'
